...
def delete_duplicate_files(files_by_hash):
    ...
    Prioriza mantener archivos en 'train' sobre 'valid' cuando están en el mismo directorio final.
    """
    deleted_files = []
    preserved_files = []
    errors = []
    ...
    for file_hash, files in files_by_hash.items():
        if len(files) > 1:
            # Separar archivos por tipo de directorio (train/valid)
            train_files = []
            valid_files = []
            other_files = []

            for file_info in files:
                path_parts = file_info["full_path"].split(os.sep)
            ...
            # Determinar qué archivos preservar y cuáles eliminar
            files_to_preserve = []
            files_to_delete = []

            if train_files and valid_files:
                # Hay archivos en ambos train y valid
                # Agrupar por directorio final (ej: late_blight, early_blight, etc.)
                train_by_final_dir = defaultdict(list)
                valid_by_final_dir = defaultdict(list)

                for file_info in train_files:
                    final_dir = os.path.basename(file_info["directory"])
                    train_by_final_dir[final_dir].append(file_info)

                for file_info in valid_files:
                    final_dir = os.path.basename(file_info["directory"])
                    valid_by_final_dir[final_dir].append(file_info)

                # Para cada directorio final, si hay archivos en train y valid, eliminar los de valid
                for final_dir in train_by_final_dir.keys():
                    if final_dir in valid_by_final_dir:
                        # Preservar uno de train, eliminar todos los de valid en este directorio
                        files_to_preserve.append(train_by_final_dir[final_dir][0])
                        files_to_delete.extend(
                            train_by_final_dir[final_dir][1:]
                        )  # Otros de train
                        files_to_delete.extend(
                            valid_by_final_dir[final_dir]
                        )  # Todos los de valid
                        print(
                            f"Eliminando archivo(s) de valid (manteniendo train): {final_dir}"
                        )
                    else:
                        # Solo hay en train para este directorio final
                        files_to_preserve.append(train_by_final_dir[final_dir][0])
                        files_to_delete.extend(train_by_final_dir[final_dir][1:])

                # Procesar directorios que solo están en valid
                for final_dir in valid_by_final_dir.keys():
                    if final_dir not in train_by_final_dir:
                        files_to_preserve.append(valid_by_final_dir[final_dir][0])
                        files_to_delete.extend(valid_by_final_dir[final_dir][1:])

            elif train_files:
                # Solo hay archivos en train
                files_to_preserve.append(train_files[0])
                files_to_delete.extend(train_files[1:])

            elif valid_files:
                # Solo hay archivos en valid
                files_to_preserve.append(valid_files[0])
                files_to_delete.extend(valid_files[1:])

            # Manejar otros archivos (que no están en train ni valid)
            if other_files:
                files_to_preserve.extend(other_files[:1])
                files_to_delete.extend(other_files[1:])

            # Agregar archivos preservados
            preserved_files.extend(files_to_preserve)
...
def generate_deletion_report(
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write("PRIORIDAD: TRAIN > VALID > OTROS\n")
        f.write("=" * 80 + "\n")
        f.write(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        if deleted_files:
            ...
            # Separar por tipo de eliminación
            valid_deleted = []
            train_deleted = []
            other_deleted = []
            ...
            for file_info in deleted_files:
                path_parts = file_info["full_path"].split(os.sep)
                if "valid" in path_parts:
                    valid_deleted.append(file_info)
                elif "train" in path_parts:
                    train_deleted.append(file_info)
                else:
                    other_deleted.append(file_info)

            if valid_deleted:
                f.write(
                    f"\nArchivos eliminados de VALID (priorizando TRAIN): {len(valid_deleted)}\n"
                )
                for i, file_info in enumerate(valid_deleted, 1):
                    f.write(f"{i}. {file_info['filename']}\n")
                    f.write(f"   Ruta: {file_info['full_path']}\n")
                    f.write(f"   Hash: {file_info['hash'][:16]}...\n")
                    f.write(f"   Tamaño: {file_info['size']} bytes\n\n")

            if train_deleted:
                f.write(
                    f"\nArchivos eliminados de TRAIN (duplicados adicionales): {len(train_deleted)}\n"
                )
                for i, file_info in enumerate(train_deleted, 1):
                    f.write(f"{i}. {file_info['filename']}\n")
                    f.write(f"   Ruta: {file_info['full_path']}\n")
                    f.write(f"   Hash: {file_info['hash'][:16]}...\n")
                    f.write(f"   Tamaño: {file_info['size']} bytes\n\n")
            ...
            if other_deleted:
                f.write(f"\nOtros archivos eliminados: {len(other_deleted)}\n")
                for i, file_info in enumerate(other_deleted, 1):
                    f.write(f"{i}. {file_info['filename']}\n")
                    f.write(f"   Ruta: {file_info['full_path']}\n")
                    f.write(f"   Hash: {file_info['hash'][:16]}...\n")
                    f.write(f"   Tamaño: {file_info['size']} bytes\n\n")
...
def main():
    ...
    print("PRIORIDAD: Mantener TRAIN > VALID > OTROS")
    print("=" * 80)
    ...
    # Analizar distribución train/valid
    train_count = 0
    valid_count = 0
    for files in duplicates.values():
        for file_info in files:
            if "train" in file_info["full_path"]:
                train_count += 1
            elif "valid" in file_info["full_path"]:
                valid_count += 1

    print(f"- Duplicados en TRAIN: {train_count}")
    print(f"- Duplicados en VALID: {valid_count}")
    ...
    if deleted_files:
        ...
        # Separar archivos eliminados por tipo
        valid_deleted = [f for f in deleted_files if "valid" in f["full_path"]]
        train_deleted = [f for f in deleted_files if "train" in f["full_path"]]

        print("\nARCHIVOS ELIMINADOS:")
        print(f"- De VALID: {len(valid_deleted)}")
        print(f"- De TRAIN: {len(train_deleted)}")
        print(
            f"- Otros: {len(deleted_files) - len(valid_deleted) - len(train_deleted)}"
        )

        if valid_deleted:
            print("\nPrimeros archivos eliminados de VALID:")
            for file_info in valid_deleted[:5]:
                print(f"  - {file_info['filename']}")

        if train_deleted:
            print("\nPrimeros archivos eliminados de TRAIN:")
            for file_info in train_deleted[:5]:
                print(f"  - {file_info['filename']}")
